"""
DeepSeek Service
封装 DeepSeek API（通过 OpenAI SDK），作为 Gemini 的替代方案。
"""
...
class DeepSeekService:
    """
    封装 DeepSeek API，提供与 GeminiService 相同的接口。
    使用 OpenAI SDK 兼容调用。
    """
    ...
    def __init__(self, model_name: Optional[str] = None):
        ...
        Args:
            model_name: 模型名称（可选，默认使用 settings.DEEPSEEK_MODEL_NAME）
        """
        ...
        logger.info(f"[DeepSeek] 使用模型: {self.model_name}")
    ...
    def _is_reasoning_model(self) -> bool:
        """判断当前模型是否为推理模型（R1），推理模型不支持 temperature/top_p。"""
    ...
    def _call_api(
        ...
        *,
        ...
        for attempt in range(max_retries):
            try:
                # 推理模型（R1）不支持 temperature/top_p 参数
                kwargs = {
                ...
                # R1 模型有 reasoning_content（思维链），仅记录日志
                if (
                    hasattr(choice.message, "reasoning_content")
                    and choice.message.reasoning_content
                ):
                    reasoning = choice.message.reasoning_content
                    logger.debug(f"[DeepSeek] 思维链长度: {len(reasoning)} 字符")
                ...
                if result_text:
                    ...
                    logger.info(f"[DeepSeek] 响应生成完毕 (模型: {self.model_name})")
                    return result_text
                else:
                ...
                last_error = e
                logger.warning(
                    f"[DeepSeek] 请求失败 (尝试 {attempt + 1}/{max_retries}): {e}"
                )
                if attempt < max_retries - 1:
                    time.sleep(1)
                    continue

        logger.error(f"[DeepSeek] 所有重试均失败: {last_error}")
    ...
    def chat(self, full_prompt: str) -> str:
        ...
        logger.info("[DeepSeek] 开始聊天请求...")

        # DeepSeek R1 不支持 system role，将系统指令放入 user 消息中
    ...
    def analyze_training(self, daily_report_md: str) -> str:
        ...
        if not daily_report_md or not daily_report_md.strip():
            logger.warning("[DeepSeek] 收到空数据，无法进行分析")
            return "## 📊 分析结果\n\n暂无数据，无法进行分析。"

        logger.info("[DeepSeek] 开始请求训练分析...")
...
"""
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"[DeepSeek] 训练分析失败: {error_msg}")
            return f"""## 📊 分析结果
...
"""
    ...
    def analyze_training_with_fallback(self, daily_report_md: str) -> str:
        """
        分析训练数据（兼容接口）。
        DeepSeek 无需模型降级，直接调用 analyze_training。
        """
        ...
        *,
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError("DeepSeek 返回非 JSON 内容")
            ...
            logger.warning(f"[DeepSeek] Home summary brief failed: {e}")
            return {"week": None, "month": None}
